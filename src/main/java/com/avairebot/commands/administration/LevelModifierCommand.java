...
public class LevelModifierCommand extends Command {
    ...
    public LevelModifierCommand(av av) {
        super(av, false);
    }
    ...
    private String getExperienceForLevel(GuildTransformer guildTransformer, int level) {
        return NumberUtil.formatNicely(
            av.getLevelManager().getExperienceFromLevel(guildTransformer, level)
                - av.getLevelManager().getExperienceFromLevel(guildTransformer, 0)
        ) + " XP";
    }
