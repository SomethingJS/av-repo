...
public class BotAdmin {
    ...
    public BotAdmin(av av, Set<String> botAdmins) {
        this.av = av;
     ...
     * av Central server, and if the user has the Bot Admin role.
     *
     * @param userId The user ID that should be checked if they're a role admin.
     * @return <code>True</code> if the user has the bot admin role on the av Central
     *         support server, <code>False</code> otherwise.
     */
    private AdminType getRoleAdminType(long userId) {
        Role role = av.getShardManager().getRoleById(
            av.getConstants().getBotAdminExceptionRoleId()
        );
