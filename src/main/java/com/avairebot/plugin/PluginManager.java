...
import com.avbot.plugin.translators.PluginHolderTranslator;
import com.avbot.plugin.translators.PluginLoaderTranslator;
...
public class PluginManager {
     ...
     */
    public void loadPlugins(av av) throws InvalidPluginsPathException, InvalidPluginException {
        ...
        Object deletedPlugins = av.getCache().getAdapter(CacheType.FILE).get("deleted-plugins");
        ...
        av.getCache().getAdapter(CacheType.FILE).forget("deleted-plugins");
    }
     ...
     */
    public void loadPluginsFromIndex(av av) {
        boolean shouldMigrate = false;

        try {
            for (DataRow row : av.getDatabase().newQueryBuilder(Constants.INSTALLED_PLUGINS_TABLE_NAME).get()) {
                ...
                final File pluginFile = new File(
                    av.getPluginManager().getPluginsFolder(),
                    plugin.getName() + ".jar"
                );
        ...
        if (shouldMigrate) {
            try {
                av.getDatabase().getMigrations().up();
            } catch (SQLException e) {
    ...
    public boolean unloadPlugin(Plugin plugin) {
        ...
        while (iterator.hasNext()) {
            ...
            next.unregisterPlugin(av.getInstance());
            iterator.remove();
     ...
     * file on the public av/plugin GitHub repository.
     *
     * @return A list of official plugins approved behind the team of Ava.
     */
    @SuppressWarnings("unchecked")
    public List<PluginHolder> getOfficialPluginsList() {
        Object plugins = av.getInstance().getCache().getAdapter(CacheType.MEMORY).remember("plugins", 10800, () -> {
            try {
                Connection.Response execute = Jsoup.connect("https://raw.githubusercontent.com/av/plugins/master/plugins.json")
    ...
    public final Plugin getPluginByName(String name) {
        List<PluginHolder> pluginHolders = av.getPluginManager().getOfficialPluginsList();
        ...
        for (PluginLoader pluginLoader : av.getPluginManager().getPlugins()) {
            if (pluginLoader.getName().equalsIgnoreCase(name)) {
                return new PluginLoaderTranslator(pluginLoader, av.getPluginManager().getOfficialPluginsList());
            }
        }
